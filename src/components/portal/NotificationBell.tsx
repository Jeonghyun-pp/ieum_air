...
const typeConfig: Record<string, { icon: React.ElementType; color: string }> = {
  strategy_ready: { icon: Sparkles, color: 'text-purple-500' },
  action_urgent: { icon: AlertTriangle, color: 'text-orange-500' },
  report_ready: { icon: FileText, color: 'text-blue-500' },
  competitor_alert: { icon: TrendingUp, color: 'text-emerald-500' },
  score_change: { icon: TrendingUp, color: 'text-pink-500' },
};
  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        className="relative p-2 rounded-full hover:bg-dark-highlight/50 transition-colors"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-80 max-h-96 rounded-xl bg-dark-elevated border border-border shadow-lg z-50 overflow-hidden">
          {/* Header */}
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => {
                ...
                return (
                  <div
                    key={n.id}
                    className={`px-4 py-3 hover:bg-dark-highlight/50 transition-colors cursor-pointer ${
