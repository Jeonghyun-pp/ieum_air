    ...
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-xl bg-dark-elevated">
        <div className="flex items-baseline gap-4">
          <div>
            <div className="text-3xl font-bold text-white">{diagnosis.photoCount}장</div>
            <div className="text-sm text-[#6A6A6A]">내 사진</div>
          </div>
          <div className="text-[#6A6A6A]">vs</div>
          <div>
            <div className="text-3xl font-bold text-[#B3B3B3]">{diagnosis.compSetMedianPhotos}장</div>
            <div className="text-sm text-[#6A6A6A]">비교군 중앙값</div>
          </div>
        </div>
        {diff > 0 && (
      ...
      {(diagnosis.resolutionIssues.length > 0 || diagnosis.brightnessIssues.length > 0) && (
        <div>
          <h4 className="text-sm font-semibold text-[#B3B3B3] mb-3">품질 개선 필요</h4>
          <div className="space-y-2">
            {diagnosis.resolutionIssues.map((issue, i) => (
              <div key={`res-${i}`} className="flex items-center gap-3 p-3 rounded-lg bg-dark-elevated">
                <span className="text-orange-400 text-xs px-2 py-0.5 rounded bg-orange-500/10">
                  저해상도
                </span>
                <span className="text-sm text-[#6A6A6A] truncate flex-1">
            ...
            {diagnosis.brightnessIssues.map((issue, i) => (
              <div key={`brt-${i}`} className="flex items-center gap-3 p-3 rounded-lg bg-dark-elevated">
                <span className={`text-xs px-2 py-0.5 rounded ${
                  issue.issue === 'too_dark' ? 'text-blue-400 bg-blue-500/10' : 'text-yellow-400 bg-yellow-500/10'
                }`}>
      ...
      {diagnosis.missingSpaces.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-[#B3B3B3] mb-3">추천 촬영 공간</h4>
          <div className="space-y-2">
      ...
      {diagnosis.spacesCovered.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-[#B3B3B3] mb-2">촬영된 공간</h4>
          <div className="flex flex-wrap gap-2">
            {diagnosis.spacesCovered.map((space) => (
              <span
                key={space}
                className="px-3 py-1 text-xs rounded-full bg-emerald-500/10 text-emerald-400"
              >
                {SPACE_LABELS[space] ?? space}
              </span>
            ))}
