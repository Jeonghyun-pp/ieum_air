...
export function LiveDemo() {
  ...
  return (
    <section className="py-24 px-4 bg-dark-base text-white overflow-hidden">
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-4 p-4 rounded-xl border transition-all duration-200 text-left ${
                    isSelected
                      ? 'bg-dark-highlight border-purple-500/50 shadow-lg shadow-purple-500/10'
                      : 'bg-dark-elevated border-transparent hover:bg-dark-highlight'
                  }`}
                >
                  <div className="text-center min-w-[48px]">
                    <div className="text-xs text-gray-500">{evt.day}</div>
                    <div className="text-lg font-bold">{evt.date}</div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium truncate">{evt.event}</div>
                  </div>
                  <div className={`text-sm font-bold ${
                    evt.adjustment === '유지' ? 'text-gray-400' : 'text-emerald-400'
                  }`}>
                    {evt.adjustment}
                  </div>
                </button>
          ...
          >
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className={`p-8 rounded-2xl bg-dark-elevated border ${config.border}`}
              >
                <div className={`w-14 h-14 rounded-2xl ${config.bg} flex items-center justify-center mb-6`}>
                  <Icon className={`w-7 h-7 ${config.color}`} />
                </div>
                <div className="text-sm text-gray-500 mb-1">{event.date} ({event.day})</div>
                <h3 className="text-2xl font-bold mb-4">{event.event}</h3>

                <div className="space-y-4">
                  <div className="p-4 rounded-xl bg-dark-highlight">
                    <div className="text-sm text-gray-400 mb-1">추천 가격 조정</div>
                    <div className={`text-3xl font-bold ${
                      event.adjustment === '유지' ? 'text-gray-400' : 'text-emerald-400'
                    }`}>
                      {event.adjustment}
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-dark-highlight">
                    <div className="text-sm text-gray-400 mb-1">추천 근거</div>
